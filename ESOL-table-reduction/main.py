...
if __name__ == "__main__":
    ...
    output_path = r"D:\materproject\single-rep-rd\table\GO_qdots"
    ...
    for n_components in [2, 5, 10, 20]:
        ...
        if n_components == 2:
            ...
            plot_clustering_result(
                ...
                f'PCA 2D Projection -  Label Spreading Clustering ({len(np.unique(cluster_labels))} clusters)'
            )
        ...
        for n_components in [2, 3]:
            ...
            if n_components == 2:
                ...
                plot_clustering_result(
                    ...
                    f't-SNE 2D Projection (perplexity={perplexity}) -  Label Spreading Clustering ({len(np.unique(cluster_labels))} clusters)'
                )
            ...
            for n_components in [2, 5]:
                ...
                if n_components == 2:
                    ...
                    plot_clustering_result(
                        ...
                        f'UMAP 2D Projection (n_neighbors={n_neighbors}, min_dist={min_dist}) -  Label Spreading Clustering ({len(np.unique(cluster_labels))} clusters)'
                    )
...
def run_vae_analysis(X_scaled, energy_values, energy_bins, output_path):
    ...
    for latent_dim in [2, 5, 10, 20, 32]:
        for intermediate_dim in [128, 256, 512]:
            ...
            # Apply  label spilling clustering
            print(
            ...
            if latent_dim == 2:
                ...
                plot_clustering_result(
                    ...
                    f'VAE 2D Projection (latent_dim={latent_dim}, intermediate_dim={intermediate_dim}, epochs={epochs}) -  Label Spreading Clustering ({len(np.unique(cluster_labels))} clusters)'
                )
        ...
        for intermediate_dim in [128, 256, 512]:
            ...
            if encoding_dim == 2:
                ...
                plot_clustering_result(
                    ...
                    f'AE 2D Projection (encoding_dim={encoding_dim}, intermediate_dim={intermediate_dim}) -  Label Spreading Clustering ({len(np.unique(cluster_labels))} clusters)'
                )
...
def create_comprehensive_visualizations(results_df, clustering_df, output_path):
    ...
    plot_comparison_bar(
        ...
        'Label Spreading Clustering Silhouette Score Comparison', output_path,
